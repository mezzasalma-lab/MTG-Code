...
**Testado com goldfish dedicado (`prismatic_bridge_goldfish_v1.py`, n=2000, 3 taxas de remoção assumidas — ver `goldfish-log.md`): incluir Greater Auramancy (2º shroud-granter) melhora a sobrevivência da Bridge em menos de 1,5 ponto percentual, em qualquer cenário testado.** Não é porque shroud não empilha (isso já era esperado) — é porque **só 14,7% das partidas chegam a ter Sterling Grove em campo em 10 turnos, e só 2,4% chegam a ter os dois protetores em campo** (mesmo com Greater Auramancy no deck). A redundância só importa nesses 2,4% dos jogos. O gargalo real é a baixa taxa de conseguir sequer 1 protetor em campo, não a falta de um segundo. **Não recomendo Greater Auramancy pra esse objetivo específico** — se proteção da Bridge for prioridade, um tutor de encantamento (pra buscar Sterling Grove de forma confiável) atacaria o problema real; adicionar mais um card de baixa densidade não ataca.
