...
VARIANTE FISICA — le `lista-fisica.md` (o deck real, cruzado carta a carta
com as 6 fotos do usuario em 2026-08-29), nao `lista.md` (a lista "afinada").

Diferencas reais vs. `urdragon_goldfish_v1.py`/`lista.md`:
- FORA: Birds of Paradise, Delighted Halfling, Ruby, Daring Tracker, Ramos,
  Dragon Engine, Talisman of Impulse, Battlefield Forge, Karplusan Forest,
  Taiga — nao localizadas em nenhuma das 6 fotos.
- DENTRO: Lightning Greaves (a caixa fisica nunca trocou pelo Talisman —
  troca so decidida no goldfish/`lista.md`, nunca aplicada na caixa), Magda,
  Brazen Outlaw e Scalelord Reckoner (confirmadas pelo usuario como "sempre
  estiveram" no fisico — `lista.md` nasceu incompleto), Dragon's Hoard,
  Morophon the Boundless e Smuggler's Surprise (extras fisicas nunca
  avaliadas), Island e Watery Grave (terrenos fisicos fora do `lista.md`).
- Scalelord Reckoner, Dragon's Hoard e Smuggler's Surprise sao registros
  NOVOS neste arquivo (nao existiam no `urdragon_goldfish_v1.py` original) -
  implementados com efeito real, nao so tag, conforme Regra 3
  (`references/goldfish-sim-card-rules.md`). Ver comentario de cada `add()`.
...
# Scalelord Reckoner: NOVO registro (extra fisica confirmada pelo usuario
# como "sempre esteve" no deck, `lista.md` nasceu incompleto). Oraculo real
# (Scryfall, C17): "{3}{W}{W}, Creature - Dragon, 4/4, Flying. Whenever a
# Dragon you control becomes the target of a spell or ability an opponent
# controls, destroy target nonland permanent that player controls." Efeito
# proprio e reativo a uma acao de OPONENTE real (mirar um Dragao seu) - sem
# oponente modelado no goldfish solo, nunca dispara (mesma classe de
# 'opponent_dependent' ja usada em Smothering Tithe, documentado, nao
# inventado). Ainda assim e um corpo Dragao 4/4 voador real: conta pra
# dragon_count()/is_dragon() (alimenta Dragon Tempest/Scourge of
# Valkas/Lathliss/Miirym/Dragon's Hoard/descontos de Dragao/Roaming Throne
# tipo Dragao), so a habilidade propria fica de fora por falta de alvo.
add("Scalelord Reckoner", 5, "creature", {"dragon", "opponent_dependent"}, power=4, pips={"W": 2})
...
# Dragon's Hoard: NOVO registro (extra fisica, nunca avaliada, achada nas
# fotos do usuario). Oraculo real (Scryfall, M19/AFC/TDC): "{3}, Artifact.
# Whenever a Dragon you control enters, put a gold counter on this
# artifact. {T}: Add one mana of any color. {T}, Remove a gold counter from
# this artifact: Draw a card." Duas habilidades ATIVADAS competem pelo MESMO
# {T} (mana OU compra, nunca as duas no mesmo turno) - simplificacao
# assumida e documentada (Regra 1, precisa validacao do usuario): tratado
# aqui SEMPRE como rock de mana (tag 'rock1', igual Arcane Signet/Talisman),
# nunca gasta pra comprar - um deck faminto por fixacao 5 cores usa o
# {T} pra mana todo turno. O acumulo de contadores de ouro E rastreado
# (`dragon_hoard_gold_counters`, incrementado em dragon_enters() a cada
# Dragao que entra, nomeado ou token - oraculo real nao exige "nontoken",
# mesma leitura ja aplicada a Scourge of Valkas/Dragon Tempest) e reportado
# como valor NAO realizado (compra que a heuristica atual deixa na mesa),
# nao inventado como compra automatica. Artefato, nao criatura - sem doenca
# de invocacao (CR 302.6), mana disponivel no mesmo turno que entra. NAO e
# criatura Dragao (e um artefato QUE REAGE a Dragoes, oraculo nao da o tipo
# Dragao a ele mesmo) - Roaming Throne (so dobra gatilho de CRIATURA do tipo
# escolhido) nunca dobra o proprio "whenever a Dragon enters" dele, checado
# explicitamente e documentado aqui pra nao faltar essa distincao depois.
...
# Smuggler's Surprise: NOVO registro (extra fisica, nunca avaliada). Oraculo
# real (Scryfall, OTJ #180/#345): "{G}, Instant - Spree (Choose one or more
# additional costs.) + {2} - Mill four cards. You may put up to two
# creature and/or land cards from among the milled cards into your hand.
...
def dragon_enters(state: GameState, name: str, is_token: bool):
    ...
    if "Dragon's Hoard" in state.battlefield:
        # "Whenever a Dragon you control enters, put a gold counter on this
        # artifact" - sem qualificador "nontoken" no oraculo real, mesma
        # leitura ja usada pro dmg_sources logo abaixo (Scourge/Tempest).
        # Dragon's Hoard e' um ARTEFATO, nao criatura - Roaming Throne (so
        # dobra gatilho de CRIATURA do tipo escolhido) nunca dobra este.
...
def rocks_mana(state: GameState) -> int:
    ...
    if "The Great Henge" in state.battlefield:
        ...
        total += 2
    if "Dragon's Hoard" in state.battlefield:
        # "{T}: Add one mana of any color." Simplificacao documentada no
        # add() dela: sempre usada pra mana (a habilidade de compra
        # concorrente pelo mesmo {T} fica de fora, nao gasta contador).
        total += 1
    return total
...
def resolve_instant_sorcery(state: GameState, name: str):
    tags = CARD_DB[name].tags
    if "dragon_cheat_hand" in tags:
        # Smuggler's Surprise, modo "{4}{G}": "You may put up to two
        # creature cards from your hand onto the battlefield." Sem
        # restricao de tipo real - qualquer criatura, priorizado por maior
        # mv (mesma logica ja usada em Bladewing/Haunting Voyage pra
        # escolher o melhor alvo disponivel). A propria Smuggler's Surprise
        # ja foi removida da mao em cast_card() antes desta chamada.
        creatures_in_hand = sorted([c for c in state.hand if is_creature_card(c)],
                                    key=lambda n: CARD_DB[n].mv, reverse=True)
...
def build_library():
    lib = []
    text = open("lista-fisica.md").read()
