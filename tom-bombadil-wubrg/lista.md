# Tom Bombadil — WUBRG (Sagas + movimentação de marcadores) — Bracket 3

> Montado em 2026-09-22 a pedido do usuário, a partir do deck
> "Tom "The Bomb" Bombadil" (Moxfield `r66vWww2dUSE8OBUy-o3aw`, atualizado
> 2026-08-05 — a base que o usuário achou mais coerente; 57 cartas mantidas)
> + EDHREC (1.799 decks de Tom no Bracket 3/"Upgraded") + o deck antigo
> (`5UwUj17670mCLXgw5b4E-A`). Sem restrição de orçamento (proxy).
> Oráculo de todas as 100 cartas conferido ao vivo no Scryfall.
> **3 Game Changers** (limite do Bracket 3): Serra's Sanctum, Enlightened
> Tutor, Teferi's Protection. Raciocínio carta a carta, motores e cortes
> em `construcao.md`.

## Comandante

1 Tom Bombadil

## Deck

1 The Kami War // O-Kagachi Made Manifest
1 Binding the Old Gods
1 There and Back Again
1 Kiora Bests the Sea God
1 Summon: Bahamut
1 Battle at the Helvault
1 War of the Last Alliance
1 Song of Eärendil
1 The Creation of Avacyn
1 Awaken the Honored Dead
1 The Coming of Galactus
1 Jugan Defends the Temple // Remnant of the Rising Star
1 Elspeth Conquers Death
1 The Eldest Reborn
1 The Bath Song
1 The Cruelty of Gix
1 Birth of the Imperium
1 In the Darkness Bind Them
1 Summon: Knights of Round
1 Summon: Primal Odin
1 Summon: Fenrir
1 Fable of the Mirror-Breaker // Reflection of Kiki-Jiki
1 The First Iroan Games
1 Summon: Yojimbo
1 Resourceful Defense
1 Goldberry, River-Daughter
1 Nexus Mentality
1 Power Conduit
1 Scholar of New Horizons
1 O'aka, Traveling Merchant
1 Hex Parasite
1 Clockspinning
1 Satsuki, the Living Lore
1 Barbara Wright
1 Flux Channeler
1 Ripples of Potential
1 Weaver of Harmony
1 Strionic Resonator
1 Estrid's Invocation
1 Narci, Fable Singer
1 Historian's Boon
1 Femeref Enchantress
1 Sythis, Harvest's Hand
1 Enchantress's Presence
1 Setessan Champion
1 Eidolon of Blossoms
1 Sol Ring
1 Arcane Signet
1 Utopia Sprawl
1 Fertile Ground
1 Sanctum Weaver
1 Bloom Tender
1 Faeburrow Elder
1 Enduring Vitality
1 Farseek
1 Prismatic Omen
1 Replenish
1 Resurgent Belief
1 Starfield of Nyx
1 Swords to Plowshares
1 Leyline Binding
1 Teferi's Protection
1 Enlightened Tutor

## Terrenos

1 Nesting Grounds
1 Karn's Bastion
1 Urza's Saga
1 Serra's Sanctum
1 Hall of Heliod's Generosity
1 The World Tree
1 Command Tower
1 Exotic Orchard
1 City of Brass
1 Mana Confluence
1 Reflecting Pool
1 Indatha Triome
1 Ketria Triome
1 Raugrin Triome
1 Savai Triome
1 Zagoth Triome
1 Spara's Headquarters
1 Raffine's Tower
1 Ziatora's Proving Ground
1 Jetmir's Garden
1 Temple Garden
1 Breeding Pool
1 Overgrown Tomb
1 Hallowed Fountain
1 Godless Shrine
1 Stomping Ground
1 Windswept Heath
1 Flooded Strand
1 Wooded Foothills
1 Verdant Catacombs
1 Misty Rainforest
1 Forest
1 Plains
1 Island
1 Swamp
1 Mountain
